...
function localizeProgram(
  ...
  return {
    ...
    // Enriched fields
    description: (p[`description_${lang}` as keyof typeof p] as string | null) ?? p.description_en ?? null,
    admission_requirements: (p[`admission_requirements_${lang}` as keyof typeof p] as string | null) ?? p.admission_requirements ?? null,
    quota_total: p.quota_total ?? null,
    quota_international: p.quota_international ?? null,
