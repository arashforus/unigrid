    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">Manage degree programs and tuition fees.</p>
        </div>
        <button
          onClick={openCreate}
        ...
        {isLoading ? (
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {programs?.map((p) => {
                ...
                return (
                  <tr key={p.id} className="border-b border-border last:border-0 hover:bg-secondary/40 transition-colors">
                    <td className="px-5 py-3.5 font-medium">{p.name_en}</td>
                    <td className="px-5 py-3.5 text-muted-foreground">{p.university_name}</td>
                    <td className="px-5 py-3.5"><span className="text-xs font-semibold px-2.5 py-1 rounded-full bg-secondary capitalize">{p.degree_type}</span></td>
                    <td className="px-5 py-3.5 text-muted-foreground">{p.language}</td>
                    <td className="px-5 py-3.5 text-muted-foreground">
                      {fee?.international_fee ? `${parseFloat(fee.international_fee).toLocaleString()} ${fee.currency}` : '—'}
                    </td>
              ...
              {programs?.length === 0 && (
                <tr><td colSpan={7} className="px-5 py-10 text-center text-muted-foreground">No courses yet.</td></tr>
              )}
            </tbody>
          </table>
        )}
